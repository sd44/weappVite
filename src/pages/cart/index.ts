...
Page({
  data: {
    cartGroupData: null as CartData | null,
  },
  ...
  refreshData() {
    this.getCartGroupData().then((res) => {
      ...
      for (const store of cartGroupData.storeGoods) {
        ...
        if (!store.shortageGoodsList) store.shortageGoodsList = [] // 该门店已加购商品如果库存为0需单独分组
  ...
  findGoods(spuId: string, skuId: string) {
    let currentStore: CartStoreGoods | undefined
    let currentActivity: Promotion | undefined
    let currentGoods: GoodsPromotion | undefined
  ...
  getCartGroupData() {
    ...
    if (!cartGroupData) return fetchCartGroupData(null)
  ...
  selectGoodsService({
    ...
    this.findGoods(spuId, skuId).currentGoods!.isSelected = isSelected ? 1 : 0
    return Promise.resolve()
  },

  // 全选门店
  // 注：实际场景时应该调用接口更改选中状态
  selectStoreService({ storeId, isSelected }: { storeId: string; isSelected: boolean }) {
    const currentStore = this.data.cartGroupData!.storeGoods.find((s) => s.storeId === storeId)
    currentStore!.isSelected = isSelected
    for (const activity of currentStore!.promotionGoodsList) {
      for (const goods of activity.goodsPromotionList) goods.isSelected = isSelected ? 1 : 0
    }
    return Promise.resolve()
  },
  ...
  // 删除加购商品
  // 注：实际场景时应该调用接口
  deleteGoodsService({ spuId, skuId }: { spuId: string; skuId: string }) {
    function deleteGoods(group: GoodsPromotion[]) {
      for (const gindex in group) {
        if (Object.hasOwn(group, gindex)) {
          const goods = group[gindex]
          if (goods.spuId === spuId && goods.skuId === skuId) {
            group.splice(Number(gindex), 1)
            return gindex
          }
        }
      }
    ...
    for (const store of storeGoods) {
      for (const activity of store.promotionGoodsList) {
        if (deleteGoods(activity.goodsPromotionList) > -1) return Promise.resolve()
      }
      if (deleteGoods(store.shortageGoodsList) > -1) return Promise.resolve()
    }
    if (deleteGoods(invalidGoodItems) > -1) return Promise.resolve()
  ...
  onGoodsSelect(e: WechatMiniprogram.CustomEvent) {
    const {
      goods: { spuId, skuId },
      isSelected,
    } = e.detail
  ...
  onStoreSelect(e: WechatMiniprogram.CustomEvent) {
    const {
      store: { storeId },
      isSelected,
    } = e.detail
  ...
  onGoodsDelete(e: WechatMiniprogram.CustomEvent) {
    const {
      goods: { spuId, skuId },
    } = e.detail
    Dialog.confirm({
  ...
  onSelectAll(event: WechatMiniprogram.CustomEvent) {
  ...
  onToSettle() {
    const goodsRequestList: GoodsPromotion[] = []
    for (const store of this.data.cartGroupData!.storeGoods) {
      for (const promotion of store.promotionGoodsList) {
        for (const m of promotion.goodsPromotionList) {
