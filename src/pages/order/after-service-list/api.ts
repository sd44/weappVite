/* eslint-disable */
import type { AfterServiceItem, AfterServiceListResponse } from "~/types/order"
import { mockIp, mockReqId } from "~/utils/mock"

export const resp: AfterServiceListResponse = {
  data: {
    pageNum: 1,
    pageSize: 10,
    totalCount: 51,
    ...
    dataList: [
      {
        ...
        refundMethodList: [
          {
            refundMethodAmount: 99_999,
            refundMethodName: "微信支付",
          },
          {
            refundMethodAmount: 100_000,
...
export type GetRightsListParams = {
...
export function getRightsList({
  ...
  parameter: GetRightsListParams
}): Promise<AfterServiceListResponse> {
  const _resq = JSON.parse(JSON.stringify(resp)) as AfterServiceListResponse
  if (parameter.pageNum > 3) {
    _resq.data.dataList = []
  }
  if (parameter.afterServiceStatus > -1) {
    _resq.data.dataList = _resq.data.dataList.filter(
      (item: AfterServiceItem) => item.rights.rightsStatus === parameter.afterServiceStatus
    )
  }
