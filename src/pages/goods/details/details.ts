...
Page({
  ...
  getSkuItem(specList: SpecItem[], selectedSku: StringObject) {
  ...
  // 获取已选择的sku名称
  getSelectedSkuValues(specTree: SpecItem[], selectedSku: StringObject) {
    const normalizedTree: Record<string, SpecValue[]> = this.normalizeSkuTree(specTree)
  ...
  normalizeSkuTree(specTree: SpecItem[]): Record<string, SpecValue[]> {
    const normalizedTree: Record<string, SpecValue[]> = {}
    for (const treeItem of specTree) {
  ...
  processSkuList(skuList: SkuItem[]) {
