...
import type { ConfirmGoods, Coupon, GoodsData, SettleDetailResponse } from "../someTypes"

export const transformGoodsDataToConfirmData = (goodsDataList: GoodsData[]): ConfirmGoods[] => {
  const list: ConfirmGoods[] = goodsDataList.map((goodsData) => ({
...
export function genSettleDetail(params: {
  ...
  couponList?: Coupon[]
  goodsRequestList: GoodsData[]
}): SettleDetailResponse {
  const { userAddressReq, couponList, goodsRequestList } = params

  const resp: SettleDetailResponse = {
    data: {
      settleType: 0,
      userAddress: null,
      totalGoodsCount: 3,
  ...
  // 判断是否携带优惠券数据
  const discountPrice: { type: number; value: number }[] = []
  ...
  const totalDiscountPrice =
    discountPrice.length > 0
      ? discountPrice.reduce((pre, cur) => {
          ...
          return pre + (cur as unknown as number)
        }, 0)
      : 0
